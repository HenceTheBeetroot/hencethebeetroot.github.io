...
export default class ParticleSystemObject extends SceneObject {
  ...
  resetParticles() {
    ...
    this._device.queue.writeBuffer(this._timeBuffer, 0, this._time[0]);
  }
  ...
  async createShaders() {
    ...
    this._particlePipelineLayout = this._device.createPipelineLayout({
      label: "Pipeline Layout",
      bindGroupLayouts: [ this._particleBindGroupLayout ]
    });

    this._uniformPipelineLayout = this._device.createPipelineLayout({
  ...
  async createPipelines() {
    this._particlePipeline = this._device.createRenderPipeline({
      label: "Particle Render Pipeline " + this.getName(),
      layout: this._particlePipelineLayout,
      vertex: {
    ...
    this._uniformPipeline = this._device.createRenderPipeline({
      label: "Uniform Render Pipeline ",
      layout: this._uniformPipelineLayout,
      vertex: {
        module: this._shaderModule, 
        entryPoint: "vertexMain",
      },
  ...
  compute(pass) { 
    let bindGroup = this._particleBindGroups[this.step];
