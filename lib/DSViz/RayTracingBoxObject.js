...
export default class RayTracingBoxObject extends RayTracingObject {
  ...
  async createComputePipeline() {
    ...
    // Create a compute pipeline that updates the image.
    this._computeFisheyePipeline = this._device.createComputePipeline({
      label: "Ray Trace Box Projective Pipeline " + this.getName(),
      layout: this._pipelineLayout,
      compute: {
        module: this._shaderModule,
        entryPoint: "computeFisheyeMain",
      }
    });
  }
  ...
  compute(pass) {
    ...
    } else if (this._camera?._mode == "fisheye") {
      pass.setPipeline(this._computeFisheyePipeline);        // set the compute fisheye pipeline
    }
    else {
